...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "Three modes in the playground. Reactive is the textbook. Memory is the textbook's star, which is also the plain Sequence of BehaviorTree.CPP. Keep holds its memory even across the answer that ends the walk: a Failure on a Sequence, a Success on a Fallback. BehaviorTree.CPP ships it for Sequence only. Put memory on the root, play, drop the battery. The safety check is on the left of the running child, so it is never asked, and the drone dies." },
...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "Below, Charge is wrapped in retry 3 and the flight home in timeout 200 ticks. Add a gust from the west and the flight home slows into the wind; when two hundred ticks pass the timeout halts it and answers Failure, the delivery fails, and the root is asked again." },
...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "Two trees below. One types the waypoint in: FlyTo A. The other reads it: FlyTo Goal. Play either, then move the goal. The typed one flies to A and then cannot finish: Drop only succeeds at the goal, and the goal is B now. It sits there dropping nothing, ten times a second. The reading one turns, because the goal lives in the world and the tree looks at it every tick." },
