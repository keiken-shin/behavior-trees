...
export const LESSONS = [
{
  ...
  flow: [
    ...
    { t: "videos" },
    { t: "aside", text: "The interpreter that runs every tree in this course is about 150 lines, and it is in the repository as src/bt/tree.js. There is no other engine behind the scenes. If the tree on the screen does something, it is because that file did it." },
  ],
},
{
  ...
  flow: [
    ...
    { t: "p", text: "Step through it. FlyTo answers Running on the first tick, and Running again on every tick after it, until it answers Success. Only then does the tick reach Land. Two actions, one after the other, and the second one waits for the first to say it is done. Once it unlocks, step it one tick at a time." },
...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "Step through the scene below. It starts with a check, battery above 30 percent, and the list gets moving. When the battery drops, the check fails, so the Sequence fails, and on the next tick the list is read again from its first child, which fails again. Nothing skipped ahead. Once it unlocks, try the memory variant and see what changes." },
...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "Below, plan A is Charge and it fails because the drone is out over the map. Plan B flies home. Watch plan A: it is asked on every tick, answers Failure on every tick, and the moment the drone is on the pad it answers Running instead, and plan B is never asked again. Once it unlocks, flip to memory and count again." },
...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "Step through the scene below: out over the map, the battery drops. The delivery branch goes idle mid flight, the return branch lights up, and the drone turns. Keep it running and it charges; because the safety branch then fails again, the delivery starts over. Nothing in the tree said abort. The tick just stopped arriving." },
...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "In the scene above, once it unlocks, pick patrol A then B, reactive. This is the counter case, because memory is not evil: the drone reaches A and starts for B, and a few ticks later it is more than three metres from A. On that tick FlyTo A is asked again, answers Running, and drags it back. It never reaches B, with or without a gust. With memory, A stays done and the drone lands at B." },
    ...
    { t: "aside", text: "Test yours. Every mode in this course is asserted by scripts/check-bt.mjs against the textbook's algorithms and the library's table, and every scene runs that same code." },
  ],
},
{
...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "Below, a Parallel with a threshold of two over three children, on the pad with the battery part empty. Hover never finishes, Charge fills the battery, and Land is done at once. Step through it and watch every child asked on every tick until two have succeeded." },
...
},
{
  ...
  flow: [
    ...
    { t: "p", text: "The scene below runs the tree that reads the goal from the blackboard: FlyTo Goal. Watch the goal move and the drone turn, with nothing in the tree changed. Once it unlocks, switch to the variant that types the waypoint in, FlyTo A, and move the goal again. It flies to A and cannot finish: Drop only succeeds at the goal, and the goal is B now. It sits there dropping nothing, ten times a second." },
    ...
    { t: "videos" },
    { t: "aside", text: "The blackboard readout under both panes is a line of keys and values, the world's view of itself, and the write log beneath it is every change the tree made this tick. Between them there is nothing hidden." },
  ],
},
{
